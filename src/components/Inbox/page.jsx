...
const Page = () => {
  ...
  const fetchEmailDetails = async (threadId, token) => {
    ...
    try {
      ...
      const result = await response.json();
      console.log("Fetched Email Details:", result); 
      if (Array.isArray(result.data)) {
      ...
      setError(`Error fetching email details: ${error.message}`);
    }
  };

  useEffect(() => {
    console.log("Updated selectedEmail:", selectedEmail);
  }, [selectedEmail]);
  ...
  const handleEmailClick = (threadId) => {
    ...
    fetchEmailDetails(threadId, token);
  };

  return (
    <div className="flex w-full h-full">
      <div className="flex w-[305px] border-[1px] border-[#343A40] h-full items-center justify-center">
              ...
              <div className="flex w-[256px] justify-between items-center">
                <div className="flex w-[129px] h-[26px] gap-1 items-center justify-between">
                  <div className="w-[34px] h-[26px] bg-[#222426] rounded-[17px] px-[8px] py-[3px] gap-2 items-center">
                    <p className="w-[18px] h-[20px] gap-1 text-[#5C7CFA] font-semibold text-[14px] text-center">
                      {emails.length}
                    </p>
                  </div>
      ...
      <div className="w-[899px] h-full">{open ? <EmailDetails selectedEmail = {selectedEmail} /> : <EmptyState />}</div>
      <div className="flex h-full border-[1px] border-[#353533] gap-4 py-4">
